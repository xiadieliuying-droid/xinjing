...
export const HistoryView = ({ onBack, onSelectResult, user }: { 
  ...
  React.useEffect(() => {
    const loadHistory = async () => {
      ...
      // 2. Load from server if user is logged in
      if (user) {
        try {
          const res = await fetch('/api/results');
          if (res.ok) {
            const data = await res.json();
            // Merge server results, avoiding duplicates if possible (using testId + date or similar)
            // For simplicity, we'll just prepend server results and filter out any that might be identical
            const serverResults = data.results;
            combinedHistory = [...serverResults, ...combinedHistory.filter(local => 
              !serverResults.some((server: any) => server.testId === local.testId && server.date === local.date)
            )];
          }
        } catch (e) {
